package com.ccb.job.core.rpc.netcom.jetty.server;
...
public class JettyServer {
		...
		thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					// Start the server
					server.start();
					logger.info(">>>>>>>>>>>> ccb-job jetty server start success at port:{}.", port);
					ExecutorRegistryThread.getInstance().start(port, ip, appName);
					server.join();	// block until thread stopped
					logger.info(">>>>>>>>>>> ccb-rpc server join success, netcon={}, port={}", JettyServer.class.getName(), port);
    			} catch (Exception e) {
					logger.error("", e);
				} finally {
					server.destroy();
	...
	public void destroy() {
		...
		logger.info(">>>>>>>>>>> ccb-rpc server destroy success, netcon={}", JettyServer.class.getName());
	}
