...
public class JobThread extends Thread{
	...
	private ConcurrentHashSet<String> triggerLogIdSet;		// avoid repeat trigger for the same TRIGGER_LOG_ID
	...
	public JobThread(IJobHandler handler) {
		...
		triggerLogIdSet = new ConcurrentHashSet<String>();
	}
	...
	public void run() {
			...
			try {
				...
				if (triggerParam!=null) {
					...
					try {
						...
						String logFileName = CcbJobFileAppender.makeLogFileName(new Date(triggerParam.getLogDateTim()), triggerParam.getLogId());

						CcbJobFileAppender.contextHolder.set(logFileName);
						CcbJobLogger.log("<br>----------- ccb-job job execute start -----------<br>----------- Params:" + Arrays.toString(handlerParams));
						...
						CcbJobLogger.log("<br>----------- ccb-job job execute end(finish) -----------<br>----------- ReturnT:" + executeResult);
					} catch (Exception e) {
						if (toStop) {
							CcbJobLogger.log("<br>----------- JobThread toStop, stopReason:" + stopReason);
						}
						...
						CcbJobLogger.log("<br>----------- JobThread Exception:" + errorMsg + "<br>----------- ccb-job job execute end(error) -----------");
					}
				...
				if (toStop) {
					CcbJobLogger.log("<br>----------- ccb-job toStop, stopReason:" + stopReason);
				}

				StringWriter stringWriter = new StringWriter();
				e.printStackTrace(new PrintWriter(stringWriter));
				String errorMsg = stringWriter.toString();
				CcbJobLogger.log("----------- ccb-job JobThread Exception:" + errorMsg);
			}
		}
		...
		logger.info(">>>>>>>>>>>> ccb-job JobThread stoped, hashCode:{}", Thread.currentThread());
	}
}
