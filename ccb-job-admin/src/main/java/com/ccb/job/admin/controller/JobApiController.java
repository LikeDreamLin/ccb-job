...
import com.ccb.job.core.biz.model.HandleCallbackParam;
...
public class JobApiController {
    ...
    @Resource
    private ICcbJobRegistryDao ccbJobRegistryDao;
    ...
    public ReturnT<String> callback(@RequestBody HandleCallbackParam handleCallbackParam){
        ...
        // valid log item
        CcbJobLog log = ccbJobLogDao.load(handleCallbackParam.getLogId());
        if (log == null) {
        ...
        if (ReturnT.SUCCESS_CODE==handleCallbackParam.getExecuteResult().getCode() && ReturnT.SUCCESS_CODE!=log.getHandleCode()) {
            CcbJobInfo ccbJobInfo = ccbJobInfoDao.loadById(log.getJobId());
            if (ccbJobInfo!=null && StringUtils.isNotBlank(ccbJobInfo.getChildJobKey())) {
                childTriggerMsg = "<hr>";
                String[] childJobKeys = ccbJobInfo.getChildJobKey().split(",");
                for (int i = 0; i < childJobKeys.length; i++) {
                    String[] jobKeyArr = childJobKeys[i].split("_");
                    if (jobKeyArr!=null && jobKeyArr.length==2) {
                        CcbJobInfo childJobInfo = ccbJobInfoDao.loadById(jobKeyArr[1]);
                        if (childJobInfo!=null) {
                            try {
                                boolean ret = CcbJobDynamicScheduler.triggerJob(String.valueOf(childJobInfo.getId()), String.valueOf(childJobInfo.getJobGroup()));
                            ...
                            childTriggerMsg += MessageFormat.format("<br> {0}/{1} 触发子任务失败, 子任务ccbJobInfo不存在, 子任务Key: {2}",
    ...
    public ReturnT<String> registry(@RequestBody RegistryParam registryParam){
        int ret = ccbJobRegistryDao.registryUpdate(registryParam.getRegistGroup(), registryParam.getRegistryKey(), registryParam.getRegistryValue());
        if (ret < 1) {
            ccbJobRegistryDao.registrySave(registryParam.getRegistGroup(), registryParam.getRegistryKey(), registryParam.getRegistryValue());
        }
        return ReturnT.SUCCESS;
    }
