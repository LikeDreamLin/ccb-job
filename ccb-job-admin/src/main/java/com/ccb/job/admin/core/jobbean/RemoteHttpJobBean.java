package com.ccb.job.admin.core.jobbean;
...
public class RemoteHttpJobBean extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext context)
		...
		CcbJobInfo jobInfo = CcbJobDynamicScheduler.ccbJobInfoDao.loadById(jobId);

		// log part-1
		CcbJobLog jobLog = new CcbJobLog();
		jobLog.setId(UUIDGenerator.getEightUUID());
		jobLog.setJobGroup(jobInfo.getJobGroup());
		jobLog.setJobId(jobInfo.getId());
		CcbJobDynamicScheduler.ccbJobLogDao.save(jobLog);
		logger.debug(">>>>>>>>>>> ccb-job trigger start, jobId:{}", jobLog.getId());
		...
		CcbJobDynamicScheduler.ccbJobLogDao.updateTriggerInfo(jobLog);

		// monitor triger
		JobFailMonitorHelper.monitor(jobLog.getId());
		logger.debug(">>>>>>>>>>> ccb-job trigger end, jobId:{}", jobLog.getId());
    }

    public ReturnT<String> doTrigger(TriggerParam triggerParam, CcbJobInfo jobInfo, CcbJobLog jobLog){
		StringBuffer triggerSb = new StringBuffer();

		// exerutor address list
		ArrayList<String> addressList = null;
		CcbJobGroup group = CcbJobDynamicScheduler.ccbJobGroupDao.load(jobInfo.getJobGroup());
